...
\begin{stresults2}
	...
	\\
	\stcmd{e(y1)} & mean of dependent variable at event-time = -1
	\\
	...
	\\
	\stcmd{e(trendmethod)} & method used to estimate the linear trend: can be ``ols'' or ``gmm''
	\\
	...
	\\
	\stcmd{e(ambiguous)} & list of cross sectional units omitted because of ambiguous event times
	\\
	...
	\\
	\stcmd{e(b\_ir)} & each column vector contains estimates of each cohort-relative-time interaction and controls included in the interaction regression.
	...
	\\
	\stcmd{e(V\_ir)} & covariance matrix of the cohort-relative-time interactions and controls included in the interaction regression.
	The interaction variables are named {\it \_interact\_m\#\_c\#} or {\it \_interact\_p\#\_c\#}, where {\it m\#} indicates {\it \#} periods before the policy change, {\it p\#} indicates {\it \#} periods after the policy change, and {\it c\#} indicates the cohort.
	Available only when {\tt cohort} and {\tt control\_cohort}, or {\tt sunabraham} are specified
	\\
	\stcmd{e(b\_interact)} & each column vector contains estimates of cohort-specific effect for the given relative time, only when \stcmd{cohort} and \stcmd{control\_cohort}, or {\tt opt sunabraham} are specified
	\\
	\stcmd{e(V\_interact)} & each column vector contains variance estimate of the cohort-specific effect estimator for the given relative time, only when \stcmd{cohort} and \stcmd{control\_cohort}, or {\tt sunabraham} are specified
	\\
	\stcmd{e(ff\_w)} & each column vector contains estimates of cohort shares underlying the given relative time, only when \stcmd{cohort} and \stcmd{control\_cohort}, or {\tt sunabraham} are specified
	\\
	\stcmd{e(Sigma\_ff)} & variance estimate of the cohort share estimators, only when \stcmd{cohort} and \stcmd{control\_cohort}, or {\tt sunabraham} are specified
	\\
